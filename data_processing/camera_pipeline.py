...
def demosaic(image):
    ...
    if batch_input:
        ...
        return out[0]
input = torch.randn((4, 48, 48))
demosaic(input).shape

def demosaic_5d(image):
    assert isinstance(image, torch.Tensor)
    ...
    for i in range(B):
        ...
        for j in range(N):
            # Generate single channel input for opencv
            im_sc = torch.zeros((num_images, image[i,j].shape[-2] * 2, image[i,j].shape[-1] * 2, 1))
            #print(im_sc.shape)
            im_sc[:, ::2, ::2, 0] = image[i, j, 0, :, :]
            im_sc[:, ::2, 1::2, 0] = image[i, j, 1, :, :]
            im_sc[:, 1::2, ::2, 0] = image[i, j, 2, :, :]
            im_sc[:, 1::2, 1::2, 0] = image[i, j, 3, :, :]
            ...
            im_t = df_utils.npimage_to_torch(im_dem_np, input_bgr=False)
            #print(im_t.shape)
            N_out.append(torch.unsqueeze(im_t, dim=0))
        bst_set = torch.cat(N_out, dim = 0) # (14, 3, 96, 96)
        B_out.append(torch.unsqueeze(bst_set, dim= 0))

    return torch.cat(B_out)
