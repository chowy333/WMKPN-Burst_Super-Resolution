...
class NET(nn.Module): #kpn(bst, feat)
    def __init__(self, args):
        ...
        # 各个卷积层定义
        # 2~5层都是均值池化+3层卷积
        #self.Upsampling = SimpleBaseline()
        ...
        self.out_kernel = nn.Sequential(
            Basic(256+64, 512, g=1, channel_att=False, spatial_att=False),
            Basic(512, 512, g=1, channel_att=False, spatial_att=False),
            nn.Conv2d(512, out_channel, 1, 1, 0)
        )

        self.out_weight = nn.Sequential(
            nn.Conv2d(256+64, 256, 1, 1, 0),
            Basic(256, 256, g=1, channel_att=False, spatial_att=False),
            nn.Conv2d(256, self.num_seq * len(self.kernel_size) * args.gt_channels, 1, 1, 0),
        )
        #(B, 12, 48, 48)
        #(B, 4, 3, 48, 48)
...
class KernelConv(nn.Module):
    ...
    def _convert_dict(self, core, batch_size, N, color, height, width):
        ...
        return core_out, bias # core_out[5] = (16, 14 ,5^2, 4, 3, 48, 48), bias = None
    ...
    def forward(self, frames, core, kernel_weight, white_level=1.0, rate=1):
        ...
        for index, K in enumerate(kernel):
            ...
            #core[K].view(B, N, -1, 3, 48, 48) (B, 14, 25*4, 3, 48, 48)
            #kernel_weight (B, 14, 4, 3, 48, 48)
            #img_stack (B, 14, 25, 4, 48, 48)
            weighted_kernel = core[K].reshape(batch_size, N, -1, 3, height, width).mul(kernel_weight[:,:,index,...].unsqueeze(dim=2)) #(B, N, 25*4, 3, 48, 48)

            for i in range(3): # RGB
                #print("wjdolkqwjdf =", core[K][:,:,:,i, ...].shape, kernel_weight[:,:,index,i,...].shape)
                weighted = weighted_kernel[...,i,:,:].view(batch_size, N, K*K, 4, height, width).mul(img_stack)
                #print("aslkdjfasd", weighted.shape)
                pred_img.append(torch.sum(torch.sum(weighted, dim=2, keepdim = False), dim =2, keepdim = False))
            pred_rgb = torch.stack(pred_img, dim=2) # (B, 14, 3, 48, 48)

            pred_img_k.append(pred_rgb)

        pred_img_i = torch.stack(pred_img_k, dim=2) # (B, 14, 4, 3, 48, 48)

        output_i = torch.mean(pred_img_i, dim=2, keepdim=False) # (B, 14, 3, 48, 48)
        output_img = torch.mean(output_i, dim =1, keepdim=False)
        #print(output_i.shape, output_img.shape)
        return output_i, output_img # yi(16 ,14, 3*4, 48, 48), y(16 ,3, 48, 48)
