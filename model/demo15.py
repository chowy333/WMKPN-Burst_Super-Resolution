...
import os
from model.common import ConvBlock_2D, ConvBlock_3D, Ref_pad, RFAB, RTAB, TCASB, CSAM
...
class NET(nn.Module):
    def __init__(self, args):
        ...
        for _ in range(num_rfab):
            m_rfab += [TCSAB(n_feats = num_seq, kernel_size=3, act_type = act_type, bias = bias, reduction = r_rate)]
        ...
        #for i in range(0, np.floor_divide(6)):
            m_trb += [Ref_pad()]
            m_trb += [TCSAB(n_feats = num_seq, kernel_size=3, act_type = act_type, bias = bias, reduction = r_rate)]
