...
class NET(nn.Module):
    def __init__(self, args):
        ...
        m_recon = [nn.PixelShuffle(scale * 2), ConvBlock_2D(16, 3, 3, act_type = None, padding = 1, bias = bias)]
        self.recon = nn.Sequential(*m_recon)
        #m_global_res = [nn.ReflectionPad2d((1,1,1,1)), RTAB(num_seq * in_channels), ConvBlock_2D(num_seq * in_channels, (scale**2)*filters, kernel_size = (3,3), padding =0, bias = bias), nn.PixelShuffle(scale*2)]
        m_global_res = [ConvBlock_2D(num_seq * filters, (scale**2)*filters, kernel_size = (3,3), padding =0, bias = bias)]
    ...
    def forward(self, x):
        ...
        #print("Final", self.tail(x).shape, global_res.shape)
        x = self.tail(x) + global_res
        output = self.recon(x)
        #print(output.shape)
        # (16, 1024, 50, 50)
        #print("Final", sr_output.shape, global_res.shape)
        return output
        #after head torch.Size([16, 64, 14, 50, 50])
        #after rfab torch.Size([16, 64, 14, 50, 50])
        #after trb torch.Size([16, 64, 2, 50, 50])
        #Final torch.Size([16, 16, 384, 384]) torch.Size([16, 16, 384, 384])
